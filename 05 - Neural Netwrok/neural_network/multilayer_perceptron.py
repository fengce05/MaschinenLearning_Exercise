...
class MultilayerPerceptron:
    def __init__(self,data,labels,layers, normalize_data = False):
        ...
        self.thetas = MultilayerPerceptron.theta_init(layers)
    ...
    def theta_init(layers):
        ...
        for layer_index in range(num_layers - 1):
            '''
            会执行两次,得到的两组参数矩阵:25*785,10*26
            '''
            in_count = layers[layer_index]
            out_count = layers[layer_index+1]
            #需要考虑偏置项，偏置的个数与输出的结果是一致的
    ...
    def cost_function(data,labels,thetas,layers):
        ...
        #制作标签，每一个样本的标签都得one-hot
        bitwise_labels = np.zeros((num_examples,mum_labels))
    ...
    def back_propagation(data,labels,thetas,layers):
        ...
        for example_index in range(num_examples):
            ...
            #逐层计算
            for layer_index in range(num_layers - 1):
                layer_theta = thetas[layer_index] #得到当前权重参数值25*785 10*26
                ...
                layers_activations[layer_index+1] = layers_activation #后裔层经过激活函数后的结果
            out_layer_activation = layers_activation[1:,:]
            ...
            # 计算输出层和真实值之间的差异
            delta[num_layers -1] = out_layer_activation - bitwise_label
